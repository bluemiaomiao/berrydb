...
import io.github.berrydb.monitor.core.DatabaseMessage;
...
public class DatabaseMessageHelper {
    ...
    public static byte[] buildDisconnectRequest(DatabaseMessage databaseMessage) throws BaseException {
        ...
        List<byte[]> fieldList = new ArrayList<byte[]>();
        fieldList.add(assembleHeader(messageLen, Operation.OP_DISCONNECT.getCode()));
    ...
    public static DatabaseMessage msgExtractReply(byte[] inBytes)
        ...
        if (Operation.OP_REPLY == databaseMessage.getOperationCode()) {
            ...
            databaseMessage.setObjList(list);
        }
        return databaseMessage;
    }

    private static List<BSONObject> extractBSONObject(byte[] inBytes) throws BaseException {
        int objLen;
        int objAllotLen;
